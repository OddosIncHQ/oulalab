...
// Importamos Routes, Route, useLocation y useNavigate para manejar la nueva página
...
import Care from './components/Care'; // <--- REFERENCIA A LA PÁGINA DE CUIDADOS
...
// Helper para resetear el scroll al entrar a /care o volver al inicio
const ScrollToTop = () => {
...
const UI_STRINGS = {
  es: {
    ...
    nav_care: 'Cuidado de Prendas', // <--- Añadido al menú
  ...
  pt: {
    ...
    nav_care: 'Cuidados com a Roupa', // <--- Añadido
...
const App: React.FC = () => {
  ...
  // Navegación inteligente (Maneja el click estando en /care o en el Home)
    ...
    <div className="min-h-screen bg-white text-gray-900 font-sans selection:bg-black selection:text-white overflow-x-hidden">
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-lg shadow-lg py-3' : 'bg-transparent py-8'}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center group cursor-pointer" onClick={() => { navigate('/'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
            <img 
          ...
          <div className="hidden md:flex items-center space-x-6 lg:space-x-10">
            ...
            {/* NUEVO LINK DE CARE (Desktop) */}
            <Link to="/care" onClick={() => setIsMenuOpen(false)} className={`text-sm font-bold uppercase tracking-widest hover:opacity-60 transition-colors ${scrolled ? 'text-black' : 'text-white'}`}>
              {t.nav_care}
            </Link>
      ...
      {/* --- EL ENRUTADOR SE ENCARGA DE MOSTRAR O LA HOME O LA GUÍA DE CUIDADOS --- */}
      <Routes>
        
        {/* RUTA 1: HOME PAGE ORIGINAL */}
        <Route path="/" element={
          <>
            {/* Hero Section */}
            ...
            {/* SECCIÓN ORIGINAL DE PLANES (Conservada Intacta) */}
        ...
        {/* RUTA 2: PÁGINA DE CUIDADOS (CARE) */}

...
class BudgetReport(models.Model):
    ...
    custom_planned_amount = fields.Float(
        readonly=True,
        string='Planned Amount (Other Currency)',
    )
    custom_practical_amount = fields.Float(
        readonly=True,
        string='Practical Amount (Other Currency)',
    )
    custom_theoritical_amount = fields.Float(
        readonly=True,
    ...
    def _get_bl_query(self, plan_fnames):
        return SQL(
            ...
            SELECT CONCAT('bl', bl.id::TEXT) AS id,
                   ...
                   0 AS committed,
                   0 AS achieved,
                   'custom_planned_amount' AS custom_line_type,
                   bl.custom_planned_amount AS custom_planned_amount,
                   0 AS custom_practical_amount,
                   0 AS custom_theoritical_amount,
                   %(plan_fields)s
              FROM budget_line bl
    ...
    def _get_aal_query(self, plan_fnames):
        return SQL(
            ...
            SELECT CONCAT('aal', aal.id::TEXT) AS id,
                   ...
                   'custom_theoritical_amount' AS custom_line_type,
                   0 AS custom_planned_amount,
                   aal.amount * budget.sign AS custom_practical_amount,
                   aal.amount * budget.sign AS custom_theoritical_amount,
                   %(analytic_fields)s
    ...
    def _get_pol_query(self, plan_fnames):
        return SQL(
            ...
            SELECT (pol.id::TEXT || '-' || ROW_NUMBER() OVER (PARTITION BY pol.id ORDER BY pol.id)) AS id,
                   ...
                   0 AS achieved,
                   'custom_practical_amount' AS custom_line_type,
                   0 AS custom_planned_amount,
                   (pol.product_qty - pol.qty_invoiced) / po.currency_rate * pol.price_unit::FLOAT * (a.rate)::FLOAT AS custom_practical_amount,
                   0 AS custom_theoritical_amount,
                   %(analytic_fields)s
